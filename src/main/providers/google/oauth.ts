...
export function getClientConfig(): ClientConfig {
  ...
  if (!clientId) throw new Error('Google sign-in is not configured. Set MAIN_VITE_GOOGLE_CLIENT_ID in .env')
  if (!clientSecret) throw new Error('Google sign-in is not configured. Set MAIN_VITE_GOOGLE_CLIENT_SECRET in .env')
 ...
 * `openUrl` opens the system browser (shell.openExternal); injected so tests need no Electron.
 */
