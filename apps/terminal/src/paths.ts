...
export function homeDir(): string {
  ...
  return join(homedir(), 'Library', 'Application Support', 'mysticals-terminal')
}
...
export function socketPath(home = homeDir()): string {
  ...
  return join(tmpdir(), `mysticals-${createHash('sha256').update(home).digest('hex').slice(0, 16)}.sock`)
}
